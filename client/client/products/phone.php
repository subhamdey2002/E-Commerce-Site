<?php
    ...
    function renderPhones($conn) {
        mysqli_select_db($conn, 'e-commerse');

        $getAllPhones = "Select * From `products` where category = 'smart phone';";

        if( $AllPhone = mysqli_query($conn, $getAllPhones) ) {

            echo "
            <div class = 'innerContainer'>
            <div class='header phone'>
            <h3>Find Your Smart Phones Here</h3>
            <button class='viewAllbtn' onclick='window.location=`http://localhost/e-commerce-site/client/renderFilteredProducts.php?filterTag=smart phone`;' >View All</button>
            </div>
            <div class='productsContainer'>";

            while( $row = mysqli_fetch_assoc($AllPhone) ) {
