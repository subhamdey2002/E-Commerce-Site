<?php
    ...
    function renderBooks($conn) {
        mysqli_select_db($conn, 'e-commerse');

        $getAllBook = "Select * From `products` where category = 'book';";

        if( $AllBook = mysqli_query($conn, $getAllBook) ) {

            echo "
            <div class = 'innerContainer'>
            <div class='header book'>
            <h3>Find Your Books</h3>
            <button class='viewAllbtn'  onclick='window.location=`http://localhost/e-commerce-site/client/renderFilteredProducts.php?filterTag=book`;'>View All</button>
            </div>
